...
SPISettings adnsSettings(125000, MSBFIRST, SPI_MODE3);
...
const int ncs0 = 10;  //This is the SPI "slave select" pin that the sensor is hooked up to
const int ncs1 = 8;  //This is the SPI "slave select" pin that the sensor is hooked up to
const int rst0 = 5;
const int rst1 = 6;
...
// Combine bytes into a SIGNED 16-bit integer explicitly
  int16_t rawX = (int16_t)((burstData[3] << 8) | burstData[2]);
  int16_t rawY = (int16_t)((burstData[5] << 8) | burstData[4]);

  // Pass the 16-bit signed values back to the pointers
  *dx = (int)rawX; 
  *dy = (int)rawY;
}
...
void dispRegisters(int ncs){
  ...
  adns_com_begin(ncs); // Use the helper instead of manual digitalWrite
